...
public class StringUtilsTest(ITestOutputHelper output) : LoggingTest(output)
{
	[Theory]
	...
	[Obsolete("Contains is obsolete")]
	public void ContainsCharTest(string text, char c, bool contains)
	{
		Assert.Equal(contains, StringUtils.Contains(text, c));
	}
	...
	{
		Assert.Equal(contains, StringUtils.Contains(text, s, comparision));
	}
